...
const Comment = ({ comments, fetchPost }) => {
    ...
    const [updatedContent, setUpdatedContent] = useState(comments.reduce((acc, comment) => {
    ...
    const handleUpdateStart = (commentId) => {
        ...
        setUpdatedContent({ ...updatedContent, [commentId]: comments.find(comment => comment.commentId === commentId).commentContents }); // 현재 댓글 내용으로 업데이트 필드 설정
    };
    ...
    const handleLike = async (commentId) => {
        setLoading(prev => ({ ...prev, [commentId]: true })); // 로딩 시작
        try {
            await axios.post(`/api/board/comment/${commentId}/like`, {}, {
                headers: { Authorization: `Bearer ${accessToken}` },
            });
            fetchPost();
        } catch (error) {
            ...
            <div className={style.commentBox}>
                {sortedComments.map((comment) => (
                    <div key={comment.commentId} className={style.commentItem}>
                        <div className={style.commentUserId}>{comment.userId}</div>
                        {isUpdating[comment.commentId] ? (
                            <input
                                className={style.editInput}
                                type="text"
                                value={updatedContent[comment.commentId]}
                                onChange={(e) => setUpdatedContent({ ...updatedContent, [comment.commentId]: e.target.value })}
                                onKeyDown={(e) => handleKeyDown(e, comment.commentId)}
                            />
                        ) : (
                            <div className={style.commentContent}>{comment.commentContents}</div>
                        )}
                        <div className={style.commentActions}>
                            {loading[comment.commentId] ? (
                                ...
                                <>
                                    <span className={style.commentCreatedDate}>{FormatDatetime(comment.commentCreatedDate)}</span>
                                    <span className={style.commentLikes}>좋아요: {comment.commentLikes}</span>
                                    <button className={style.like_Button} onClick={() => handleLike(comment.commentId)}>좋아요</button>
                                    {isUpdating[comment.commentId] ? (
