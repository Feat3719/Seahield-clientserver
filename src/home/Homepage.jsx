...
function Homepage() {
    ...
    const [imageUrl, setImageUrl] = useState(`http://192.168.0.74:8000/static/webcamapp/detect/exp/temp.jpg?${Date.now()}`);
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        const updateImage = () => {
            setLoading(true);
            const newImageUrl = `http://192.168.0.74:8000/static/webcamapp/detect/exp/temp.jpg?${Date.now()}`;
